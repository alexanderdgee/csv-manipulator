...
public class CSVUtilsTest {
    ...
    private String samplePath = FileUtils.relativePath(sampleName);

    @Test
    public void testUpdateRowValues() throws IOException {
        createSample();
        CSVUtils.updateRowValues(sampleName, "origin", "Londom", "London");
        List<String> processedLines = FileUtils.readLines(samplePath).collect(Collectors.toList());
    ...
    private void createSample() throws IOException {
        FileUtils.writeLines(samplePath, List.of(sample1, sample2, sample3, sample4, sample5));
    }
}
